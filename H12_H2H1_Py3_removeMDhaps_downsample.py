...
# November 2014
# Stanford University
# Updated, Mariana Harris 2021 mharris94@ucla.edu
# This script calculates H12 and H2/H1 from population genomic data,
# removing haplotypes driven by high values of missing data and downsampling to 265 SNPs per 401 SNP window
...
def clusterHaplotypes(inFile, outFile, windowTot, jump, distanceThreshold, numStrains):
    ...
    # downsample to a smaller number of SNPs to be comparable between populations with different SNP densities
    downSampledFlies=sampleDownWindow(flies, window)
...
#######################
def downsample(flies,n_downsample):
    flies_Ns={key:list(flies[key]).count('N') for key in flies}
    #sort number of Ns and choose top n_downsample strains
    flies_Ns_sorted=dict(sorted(flies_Ns.items(), key=lambda item: item[1]))
    flies_topN = list(flies_Ns_sorted)[:n_downsample]
    flies_list=[flies[key] for key in flies_topN]
    keys = list(range(1,n_downsample+1))
    flies =  dict(zip(keys, flies_list))
    return flies
#######################
...
def sampleDownWindow(flies, window):
    totalSNPs=range(0,int(window)*2+1)
    subsampleSNPs=random.sample(totalSNPs,sampleDownWinSize)
...
def hamming_distance_clump(s1, s2, distanceThreshold):
    ...
    lst_s1=list(s1)
    lst_s2=list(s2)
    numN_s1=lst_s1.count('N')
    numN_s2=lst_s2.count('N')
    
    if numN_s1 >= int(len(s1)*0.1) or numN_s2 >= int(len(s2)*0.1): #if hap has more than 1$
        distance =len(s1)
    else:
        distance = 0
...
def mkOptionParser():
    ...
    parser.add_option("-n", "--downsample",      type="int",       default=100,    help="downsample window size")
...
def main():
    ...
    global sampleDownWinSize
    sampleDownWinSize = int(args[2])
    print(sampleDownWinSize)
