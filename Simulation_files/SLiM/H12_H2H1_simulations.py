...
# November 2014
# Stanford University
# This script calculates H12 and H2/H1 
# Updated, Mariana Harris 2022 mharrris94@ucla.edu
# The script calculates H12 and H2/H1 from simulated data
...
def clusterSingleWindow(inFile, outFile, windowTot, distanceThreshold, numStrains, singleWindow, basePair, locusLength, PF, Nac, sel, age, numsnps,downSampleWindow,numorigins,theta, Q):
# This definiton calculates H12, H2, and H1 in a single window centered around the coordinate specified by the user. If the coordinate is not in the file or within the bounds of a defineable window, then an error message is outputted.  
    ...
    lastSNP = int(numberLines - window -1)
    ...
    if basePair >0 and numberLines >0:
        ...
        for lineNo in range(window, lastSNP):
            ...
            if coord >=50000 and center == -1: #the center of the sweep has been located
                center=lineNo
    ...
    if center !=-1:
        if downSampleWindow != -100:
            downSampledFlies=sampleDownWindow(flies, window,downSampleWindow)
            runAllDefs(downSampledFlies, center, distanceThreshold, inFile, outFile, window, center_coord, edgeCoord1, edgeCoord2, PF, Nac, sel, age, numsnps,numorigins,theta, Q)
        else:
            runAllDefs(flies, center, distanceThreshold, inFile, outFile, window, center_coord, edgeCoord1, edgeCoord2, PF, Nac, sel, age, numsnps,numorigins,theta, Q)


######################
def clusterHaplotypes(inFile, outFile, windowTot, jump, distanceThreshold, numStrains, PF, Nac, sel, age,downSampleWindow):
    ...
    if downSampleWindow != -100:
        downSampledFlies=sampleDownWindow(flies, window,downSampleWindow)
        runAllDefs(downSampledFlies, center, distanceThreshold, inFile, outFile, window, center_coord, edgeCoord1, edgeCoord2, PF, Nac, sel, age, numsnps)
    else:
    ...
    for center in range(window+1+jump, lastSNP, jump):
        ...
        if downSampleWindow != -100:
            downSampledFlies=sampleDownWindow(flies, window,downSampleWindow)
            runAllDefs(downSampledFlies, center, distanceThreshold, inFile, outFile, window)
        else:
...
def sampleDownWindow(flies, window,downSampleWindow):
    totalSNPs=range(0,int(window)*2+1)
    subsampleSNPs=random.sample(totalSNPs,downSampleWindow)
    ...
    for x in flies.keys():
        newFlies[x] = []
    for x in flies.keys():
        for y in subsampleSNPs:
            newFlies[x].append(flies[x][y])
    return newFlies

#######################

def runAllDefs(flies, center, distanceThreshold, inFile, outFile, window, center_coord, edgeCoord1, edgeCoord2, PF, Nac, sel, age, numsnps,numorigins,theta, Q):
# This definition runs all other definitions to identify haplotypes, their clusters, and summary statistics. This is run for every analysis window. 
        ...
        printClusters(inFile, outFile, center_coord, clusters, haps_clumped,  keyVector, sizeVector, absLengthWin, edgeCoord1, edgeCoord2, flies, PF, Nac, sel, age, numsnps, pi,numorigins,theta, Q)
...
######################
def printClusters(inFile, outFile, centerCoord, clusters, haps,  keyVector, sizeVector, absLengthWin, edgeCoord1, edgeCoord2, flies, PF, Nac, sel, age, numsnps, pi,numorigins,theta, Q):
...
# this definition calculates several summary statistics from the haplotypes including K (number of unique haplotypes) and haplotype homogygosity statistics H12, H1, and H2. The outputting of all summary statistics and relevant information about the analysis window is done in this definition as well. 
        ...
        outFile.write(centerCoord + '\t' + str(edgeCoord1) + '\t' + str(edgeCoord2)  + '\t' + str(K) +  '\t' + clusterSize + '\t' +  membersOfClusters + '\t' + str(H1) + '\t' + str(H2) + '\t'  + str(H12) + '\t' + str(ratioH2H1) + '\t' + str(H123) + '\t' + str(numsnps) + '\t' + str(pi)+ '\t'+ str(numorigins) + '\t'+ PF + '\t' + Nac + '\t' +  sel + '\t' + age+ '\t' + str(theta)+ '\t' + str(Q) +'\n')
...
def mkOptionParser():
    ...
    parser.add_option("-z", "--downsampleWindow",      type="int",       default=-100,    help="downsample window size in snps")
    parser.add_option("-y","--origins", type="int",default=-1,help="number of origins")
    parser.add_option("-x", "--adaptiveTheta", type="float", default='0.0', help="adaptive Theta")
    parser.add_option("-q", "--rescaling", type="int", default='-100', help="rescaling constant")
...
def main():
    ...
    downSampleWindow    = options.downsampleWindow
    numorigins = options.origins
    theta = options.adaptiveTheta
    Q = options.rescaling
    ...
    if singleWindow !=-100:
        clusterSingleWindow(inFile, outFile, windowTot, distanceThreshold, numStrains, singleWindow, basePair, locusLength, PF, Nac, sel, age, numsnps,downSampleWindow,numorigins,theta, Q)
    else:
        clusterHaplotypes(inFile, outFile, windowTot, jump, distanceThreshold, numStrains,PF, Nac, sel, age, downSampleWindow)
